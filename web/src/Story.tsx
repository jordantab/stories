...
function makeBar(dark: boolean) {
  return (
    <div
      className={"h-1 w-12 rounded " + (dark ? "bg-orange-500" : "bg-gray-300")}
    ></div>
  );
}
...
function StoryPage() {
  ...
  useEffect(() => {
    loadPage(0);
    axios
      .post(HOST + "leads", { story_id: story_id })
      .then((res) => {
  ...
  return (
    <div className="h-screen bg-slate-800 text-orange-500 flex flex-col">
      ...
      <div className="flex-grow flex flex-col justify-center space-y-4 py-8 px-20">
        {page.type === "query" ? (
          <>
            ...
            <input
              value={queryResponse}
              onChange={(ev) => setQueryResponse(ev.target.value)}
